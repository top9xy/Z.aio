    ...
    internal class EvadeTargetManager
    {
        ...
        private static void OnCreate(GameObject sender,EventArgs args)
        {
...

                {
                    bestAllies = GameObjects.AllyHeroes
                        .Where(t =>
                            t.Distance(ObjectManager.Player) < Z.aio.Common.Champion.W.Range)
                        .OrderBy(x => x.Health);
                }
                ...
                foreach (var ally in bestAllies)
                {
                    if (aaa == ally)
                    {
                        if (ObjectManager.Player.CharacterName == "Janna" ||
                            ...
                            ObjectManager.Player.CharacterName == "Karma")
                        {
                            if (Z.aio.SpellBlocking.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < Z.aio.Common.Champion.E.Range)
                            {
                                Z.aio.Common.Champion.E.CastOnUnit(aaa);
                            }
                        }
                        if (ObjectManager.Player.CharacterName == "Lux" || ObjectManager.Player.CharacterName == "Sona" ||
                            ObjectManager.Player.CharacterName == "Taric")

                        {
                            if (Z.aio.SpellBlocking.EvadeTargetManager.Menu["whitelist"][
                                        ally.CharacterName.ToLower()] &&
                                aaa.Distance(ObjectManager.Player) < Z.aio.Common.Champion.W.Range)
                            {
                                Z.aio.Common.Champion.W.CastOnUnit(aaa);
                            }
                        }
                    }
